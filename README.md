# 学生生活支援ツール開発プロジェクト (Student Utility Kit Innovation)
...
| メンバー | 役割 | 担当ファイル | 機能実装 |
|---|---|---|---|
| **A** | メインロジック（計算・判定） | `logic.c` `logic.h` | 期限順に並び替える機能 |
| **B** | データ管理（ファイルI/O） | `storage.c` `storage.h` | 不正な日付をエラーにする機能 |
| **C** | UI（メニュー・入力チェック） | `ui.c` `main.c` | タスクのジャンル分け機能 |
